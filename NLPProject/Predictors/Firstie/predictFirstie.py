...
#Code structure adopted from predictPlebe.py
...
def extract_email_features(row):
    ...
    if re.match(pattern, email):
        ...
        if date < pd.Timestamp(2022, 5, 25, 12) and emailNum == 22:
            return 1
        if date < pd.Timestamp(2023, 5, 25, 12) and emailNum == 23:
            return 1
        if date < pd.Timestamp(2024, 5, 25, 12) and emailNum == 24:
            return 1
        if date < pd.Timestamp(2025, 5, 25, 12) and emailNum == 25:
            return 1
    return 0
...
def balance_dataset(email_df):
    ...
    print(f"Balanced dataset size: {len(balanced_df)} (Firsties: {len(sampled_plebes)}, Non-Firsties: {len(sampled_non_plebes)})")
    return balanced_df


import joblib

def train_classification_model(email_df, model_file="firstie_classifier.pkl"):
...
pkl_file = '../../numStripped_combined_emails.pkl' 
