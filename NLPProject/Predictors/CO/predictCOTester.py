...
#Code structure adopted from predictMid.py
...
def evaluate_model(model_file):
    ...
    print("\nFalse Positives:")
    for i, text in enumerate(false_positives[:4]):  # Print 2 instances
    ...
    print("\nFalse Negatives:")
    for i, text in enumerate(false_negatives[:4]):  # Print 2 instances
        print(f"Instance {i + 1}:")
        print(f"Text: {text}\n")

# Load the saved model and evaluate it
evaluate_model(model_file="CO_classifier.pkl")
