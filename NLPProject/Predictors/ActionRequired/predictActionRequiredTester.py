...
#code structed adopted from predictMidTester.py
...
def evaluate_model(model_file):
    ...
    print("\nFalse Positives:")
    for i, text in enumerate(false_positives[:5]):
    ...
    print("\nFalse Negatives:")
    for i, text in enumerate(false_negatives[:5]):  
        print(f"Instance {i + 1}:")
        print(f"Text: {text}\n")

# Load the saved model and evaluate it
evaluate_model(model_file="ActionRequired_classifier.pkl")
