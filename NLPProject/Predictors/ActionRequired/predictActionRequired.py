...
def extract_email_features(row):
    ...
    if not isinstance(subjectLine, str):
        return 0
    
    pattern = r".*Action Required.*"
    if re.match(pattern, subjectLine):
        #print(subjectLine)
        return 1
...
def balance_dataset(email_df):
    ...
    """
    # Separate plebes (label 1) and non-plebes (label 0) plebes in this case actually means action required, I just did not change the variable name 
    ...
    print(f"Balanced dataset size: {len(balanced_df)} (Action Required: {len(sampled_plebes)}, Non-Action Required: {len(sampled_non_plebes)})")
    return balanced_df


import joblib

def train_classification_model(email_df, model_file="ActionRequired_classifier.pkl"):
...
pkl_file = '../../combined_emails.pkl'  # Does not use the stripped on because there is no point 
