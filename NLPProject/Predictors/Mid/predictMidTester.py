...
def evaluate_model(model_file="mid_email_classifier.pkl"):
    ...
    print("\nFalse Positives:")
    for i, text in enumerate(false_positives[:5]):  # Print 5 instances
    ...
    print("\nFalse Negatives:")
    for i, text in enumerate(false_negatives[:5]):  # Print 5 instances
        print(f"Instance {i + 1}:")
        print(f"Text: {text}\n")

# Load the saved model and evaluate it
evaluate_model(model_file="mid_email_classifier.pkl")
